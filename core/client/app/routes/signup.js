...
export default Route.extend(styleBody, {
    ...
    beforeModel() {
        ...
        if (this.get('session.isAuthenticated')) {
            this.get('notifications').showAlert('请先退出登录，然后再注册新用户。', {type: 'warn', delayed: true, key: 'signup.create.already-authenticated'});
        ...
        return new RSVP.Promise((resolve) => {
            ...
            return ajax({
                ...
                if (response && response.invitation && response.invitation[0].valid === false) {
                    this.get('notifications').showAlert('邀请不存在或已经失效。', {type: 'warn', delayed: true, key: 'signup.create.invalid-invitation'});
