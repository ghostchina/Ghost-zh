            ...
            this.store.findAll('user', {reload: true}).then(function (result) {
                ...
                if (invitedUser) {
                    if (invitedUser.get('status') === 'invited' || invitedUser.get('status') === 'invited-pending') {
                        self.get('notifications').showAlert('已经邀请了此邮箱的持有人。', {type: 'warn', key: 'invite.send.already-invited'});
                    } else {
                        self.get('notifications').showAlert('此邮箱/用户已存在。', {type: 'warn', key: 'invite.send.user-exists'});
                    }
                } else {
                    ...
                    newUser.save().then(function () {
                        ...
                        if (newUser.get('status') === 'invited-pending') {
                            self.get('notifications').showAlert('邀请邮件未能发送！请重新发送。', {type: 'error', key: 'invite.send.failed'});
                        } else {
